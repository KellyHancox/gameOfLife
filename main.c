...
//included to get rid of new line character
#include <string.h>
...
int main(int argc, char** argv){
	...
	while(1){
		...
		printf("n to iterate multiple times, or any other\n");
		...
		switch(fn[0]){
			...
			case 'w':
				...
				//created to track length of filename
				int len;

				fgets(filename, 254, stdin);

				//takes away new line character of new filename
				len = strlen(filename);
				if(filename[len-1] == '\n'){
				    filename[len-1] = 0;
				}
